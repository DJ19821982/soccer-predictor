...
class App(tk.Tk):
    ...
    def _create_widgets(self):
        ...
        self.log = tk.Text(mid, height=20)
    ...
    def fetch_fd(self):
        ...
        if not api:
            messagebox.showinfo('API key needed', 'Set FOOTBALL_DATA_API_KEY or use openfootball files.')
            return
    ...
    def show_predictions(self):
        ...
        if not preds:
            messagebox.showinfo('No upcoming matches', 'No upcoming matches found.')
            return

        win = tk.Toplevel(self)
        win.title('Predictions')
        win.geometry('700x500')

        columns = ('date','comp','home','away','prob_win','prob_draw','prob_loss','top_score')
